...
function appendLine(payload) {
  ...
  const isDamageLine = /你释放了|对.*造成.*点伤害|受到.*点伤害|闪避了.*的攻击|躲过了.*的攻击|溅射到|波及|造成.*点伤害|中了毒|中毒伤害|毒特效|毒性攻击|每秒损失.*气血|持续\d+秒|恢复.*点生命|施放.*恢复|暴击|连击触发|触发破防|触发破魔|破防攻击|破防效果|无敌状态|无敌效果|减伤效果|禁疗影响|禁疗效果|免疫了所有伤害|处于无敌|施放.*护盾|震退了怪物|施放.*，震退|施放.*，造成范围伤害|攻击落空|被麻痹戒指定身|被麻痹|无法行动|施毒成功|施毒失败|自动使用.*药/.test(text);
  if (isDamageLine) {
